...
def main():
    ...
    # # NBEATS
    # quantiles = np.sort(
    #     np.concatenate([[50 - lv / 2, 50 + lv / 2] for lv in level])[1:] / 100
    # )
    # nbeats_model = get_nbeats_model(
    #     horizon=horizon, input_size=2 * horizon, quantiles=quantiles
    # )
    ...
    Y_rec_df = reconcile_forecasts(reconcilers, Y_hat_ets, Y_fitted_ets, S_df, tags)
    ...
    # Y_rec_df = Y_rec_ets.merge(Y_rec_xgb, on=["unique_id", "ds"]).merge(
    #     Y_rec_nbeats, on=["unique_id", "ds"]
    # )

    # Evaluate forecasts
    ets_models = [col for col in Y_rec_df.columns if "AutoETS" in col]
    # xgb_models = [col for col in Y_rec_xgb.columns if "XGBRegressor" in col]
    # nbeats_models = [col for col in Y_rec_nbeats.columns if "NBEATS" in col]
    ...
    print("\nPipeline finished. Results saved to assets/results/")
